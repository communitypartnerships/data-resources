...
subtitle: "Centering Equity in Evaluating Well-Being & Quality of Life for Albemarle County Residents: Key Results"
date: "May 2021"
...
# Load combined data
load("../submodules/albequity_profile/webstory/webstory_data.RData")
...
[Download Report PDF](../pdfs/Albemarle_County_Equity_Profile_2021.pdf){.external target="_blank"}
<br />
...
The Albemarle County Equity Profile analyzes various conditions across demographic groups and geographic areas that contribute to well-being. This online version provides some key results from the full [Equity Profile](../pdfs/Albemarle_County_Equity_Profile_2021.pdf). For further analysis, reporting, and data sources, please see the report.
...
```{r fig1, out.width='100%', results = 'asis'}
race_dec <- race_dec %>% 
  filter(year != 2019)
...
p <- ggplot(race_dec_long, aes(x = year, y = pop_percent, color = poptype)) +
  ...
  theme(
    ...
    #legend.position = "top",
    panel.grid.major.y = element_line(linetype = "dashed", linewidth = .4),
...
```{r fig2, fig.height = 6, fig.width = 8, out.width='100%', results = 'asis'}
race_trends_poc <-
  alb_dems %>%
...
race_trends3 <- ggplot(race_trends_poc, aes(x=year, y=pct2, fill=final_level)) +
  geom_area(alpha=0.6 , linewidth=.5, color = "white") +
  scale_x_continuous(breaks=seq(2010,2019, 1)) +
  # scale_fill_manual(values = c(rev(poc_pal))) +
  theme_minimal() +
  labs(x="Year", y="Population %", fill = NULL)
  
  # ggplot(race_trends_poc, aes(x = year, y = pct2, fill = final_level)) +
...
```{r fig5, fig.height = 6, fig.width = 8, results = 'asis'}
ahdi_map <-
  alb_tract %>%
...
```{r fig6, fig.height=8, out.width='100%', results = 'asis'}
...
```{r fig9, fig.height = 6, fig.width = 8, results = 'asis'}
life_map <-
  alb_tract %>%
...
```{r fig13, fig.height = 6, fig.width = 8, results = 'asis'}
ed_graph <-
  ed_dist %>%
  ungroup() %>%
  group_by(Sex, Race) %>%
...
```{r fig14, fig.height = 6, fig.width = 8, results = 'asis'}
...
```{r fig18, fig.height = 6, fig.width = 8, results = 'asis'}
...
```{r fig19, fig.height = 6, fig.width = 8, out.width='100%', results = 'asis'}
alice_graph  <-
  alice_alb %>%
...
```{r fig21, fig.height = 6, fig.width = 8, out.width='100%', results = 'asis'}
alice_thresh_graph <-
  alice_thresh %>%
...
alice_thresh_gg <-
  ...
  geom_area(data = alice_thresh_graph,
            ...
            color = "black", linewidth = 0, alpha = .1,
            fill = alice_pal[3] ) +
...
The Albemarle County Equity Profile was co-authored by the [The Equity Center](https://virginiaequitycenter.org), A [UVA Democracy Initiative](https://democracyinitiative.virginia.edu/) for the Redress of Inequity through Community-Engaged Scholarship and the [Albemarle County Office of Equity and Inclusion](https://albemarle.org).
