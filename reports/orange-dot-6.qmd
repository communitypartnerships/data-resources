...
fullwidth_map = list(
  ...
  fig.retina = 2, out.width = '100%', fig.align = "center"
))
...
tract_geo <- tracts(state = "VA", county = county_codes, year = 2022)
...
selfsuff_county <- read_csv("../submodules/orange-dot/OrangeDot6_files/data/self_suff_county_2021.csv")
...
fam_ssw <- read_csv("../submodules/orange-dot/OrangeDot6_files/data/faminc_ssw_race_county_2022.csv")
...
faminc_race <- read_csv("../submodules/orange-dot/OrangeDot6_files/data/faminc_race_county_2022.csv") 
...
med_faminc_county <- read_csv("../submodules/orange-dot/OrangeDot6_files/data/med_faminc_county_2022.csv") %>%
  mutate(GEOID = as.character(GEOID),
         estimate = round(estimate, 0),
         locality = str_to_title(str_remove(locality, ", Virginia")))

# Median family income by tract ----
med_faminc_tract <- read_csv("../submodules/orange-dot/OrangeDot6_files/data/med_faminc_tract_2022.csv") %>%
  mutate(GEOID = as.character(GEOID),
...
ssw_tract <- read_csv("../submodules/orange-dot/OrangeDot6_files/data/faminc_ssw_tract_2022.csv") %>%
...
[Download Report PDF](../pdfs/orange-dot-6-final-print.pdf){.external target="_blank"}
</br>
[Download Executive Summary PDF](../pdfs/orange-dot-6-summary-print.pdf){.external target="_blank"}
...
workers_by_hours <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/workers_by_hours.csv") %>% 
  mutate(
...
family_income <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/families_below_ess_by_incomes.csv")
...
workers_by_medianwages <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/workers_by_medianwages.csv") %>% 
   mutate(
...
```{r occupations_by_wagebin, opts.label='fullwidth_table'}
occupations_by_wagebin <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/occupations_by_wagebin.csv")
...
```{r, fig.align = "center"}
knitr::include_graphics("../submodules/orange-dot/OrangeDot6_files/final_report/average-family-wealth-by-race-and-ethnicity-1963-2022.png")
```
...
fam_by_race <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/families_by_race.csv") 
...
fam_by_age <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/families_by_age.csv") %>% 
  left_join(fam_by_ess_totals) %>% 
  mutate(count_rounded = round(count, -2),
         sss_group_tot_rounded = round(sss_group_total, -2),
         percent_rounded = round(count_rounded/sss_group_tot_rounded *100, 0)) %>% 
...
fam_by_tenure <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/families_by_tenure.csv") %>% 
...
fam_by_burden <- read_csv("../submodules/orange-dot/OrangeDot6_files/microdata/data/families_by_burden.csv") %>% 
  mutate(label_text = case_when(
    below_ess_finc == "No" ~ paste0(prettyNum(count, big.mark=",", preserve.width="none"), " (", round(percent, 0), "% of families above SSS)"),
    below_ess_finc == "Yes" ~ paste0(prettyNum(count, big.mark=",", preserve.width="none"), " (", round(percent, 0), "% of families below SSS)")),
         below_ess_finc = factor(below_ess_finc, levels = c("No", "Yes"),
...
### Albemarle County {.unlisted .unnumbered}
```{r}
# Set name
...
income_map <- med_faminc_tract %>%
  ...
  ggplot() +
  annotation_map_tile(zoomin = 1, progress = "none", cachedir = "orange-dot-6_cache/") +
...
below_ss_map
```
[Return to Localities ↩︎](#locality-profiles)

### Buckingham County {.unlisted .unnumbered}
```{r, echo=FALSE}
# Set name
...
### City of Charlottesville {.unlisted .unnumbered}
```{r, echo=FALSE}
# Set name
...
### Fluvanna County {.unlisted .unnumbered}
```{r, echo=FALSE}
# Set name
...
### Greene County {.unlisted .unnumbered}
```{r, echo=FALSE}
# Set name
...
### Louisa County {.unlisted .unnumbered}
```{r, echo=FALSE}
# Set name
...
### Nelson County {.unlisted .unnumbered}
```{r, echo=FALSE}
# Set name
...
### Citation
Elizabeth Mitchell, Michele Claibourn, Ridge Schuyler. “Orange Dot Report 6.0: Family Self-Sufficiency in the Charlottesville Region: Albemarle, Buckingham, Fluvanna, Greene, Louisa, Nelson Counties & the City of Charlottesville, Virginia.” Published October 21, 2024. https://communitypartnerships.github.io/data-resources/reports/orange-dot-6/

This work is licensed under a [Creative Commons Attribution 4.0 International License](https://creativecommons.org/licenses/by/4.0/).

## Appendix

Download the [report pdf](../pdfs/orange-dot-6-final-print.pdf){.external target="_blank"} for tables on expenses and self-sufficiency wages for different family compositions for each locality in this report.